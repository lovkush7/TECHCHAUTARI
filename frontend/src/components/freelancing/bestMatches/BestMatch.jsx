import { Bookmark, LocateIcon, Save, SaveIcon } from 'lucide-react'
import React, { useState } from 'react'
import { CiLocationOff, CiLocationOn, CiSaveDown1 } from 'react-icons/ci'
...
const BestMatch = () => {
  ...
  return (
    <div className='flex flex-col flex-wrap w-full space-y-4 '>
       {jobs.filter((job)=>job.time === "today").map((job)=>(
        <div key={job.id} className='group p-1 mt-3  ml-5 mr-5 bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100 hover:shadow-lg transition-shadow duration-300'>
            <div className='flex justify-end'>
               <Bookmark className={save ? "text-red-600 fill-red-500 " : "" } onClick={toogleSave}/>
            </div>
